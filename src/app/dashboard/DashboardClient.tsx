    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={m.label} className="card px-4 py-3">
            ...
            <p className="font-serif text-3xl sm:text-4xl font-semibold tracking-tight text-tinta leading-none mb-1">{m.value}</p>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-5">
        <div className="flex gap-2">
          ...
          <Link href="/cadastros" className="btn-secondary sm:hidden">
            Cadastros
          </Link>
        </div>
        <input
        ...
        />
        <Link href="/cadastros" className="btn-secondary ml-auto hidden sm:block">
          Cadastros
        </Link>
      </div>
      ...
      <div className="flex items-center gap-1 border-b border-linha mb-5 overflow-x-auto">
        {["Todos", ...statusOpcoes.map(s => s.nome)].map((tab) => (
          <button
            ...
            className={`px-3 sm:px-4 py-2.5 text-sm whitespace-nowrap border-b-2 transition-colors -mb-px ${
              activeStatus === tab
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((pedido) => (
            <div key={pedido.id} className="card p-4 sm:p-5 hover:border-cinza-poeira transition-colors">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  {pedido.mes && (
                    <p className="text-[10px] uppercase tracking-widest text-cinza-poeira mb-1">{pedido.mes}</p>
                  )}
                  <h3 className="font-serif text-lg font-semibold leading-tight text-tinta pr-2">
                    {pedido.clinica}
                  </h3>
                </div>
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-cinza-poeira mb-3">
                {pedido.editor_nome && <span>✏️ {pedido.editor_nome}</span>}
